...
public class DateUtils {
     ...
     * 日期格式转化
     *
     * @param srcDate    源日期字符串
     * @param srcFormat  源日期格式，默认为yyyy-MM-dd
     * @param destFormat 目的日期格式，默认为yyyy-MM-dd
    ...
    public static String secondToDateFormat(long second, String format) {
        GregorianCalendar gc = new GregorianCalendar();
        gc.setTimeInMillis(second * 1000);
        return dateFormat(gc.getTime(), format);
    }
     ...
     * 将指定的日期转换成字符串
     *
     * @param aDteValue 要转换的日期
     * @param aFmtDate  转换后日期的格式，默认为yyyy-MM-dd
     ...
     * 将指定的字符串转换成日期
     *
     * @param aStrValue 要转换的字符串
     * @param aFmtDate  转换日期的格式，默认为yyyy-MM-dd
     * @param aDteRtn   转换之后的字符串
     ...
     *
     * @param yearmonth
     * @return modify by shibc 20090402
     * @version: V1.0
     */
     ...
     *
     * @param time1 起始时间串 必须是yyyy-MM-dd HH:mm:ss格式
     * @param time2 终止时间串 必须是yyyy-MM-dd HH:mm:ss格式
     * @return true-当前时间在tiem1与time2之间，false-当前时间不在tiem1与time2之间
     * @version: V1.0
     */
     ...
     * 根据指定日期获得上一个月的月份
     *
     * @param dataStr 制定的日期,可为空，空为当前时间
     * @param srcFormat 指定日期的format
     * @param tarFormat 需要返回的日期格式
     * @param addMonths 增加的月份
     * @return 指定日期的上个月的月份
     * @version: V1.0
     */
    public static String getAddMonth(String dataStr, String srcFormat, String tarFormat, int addMonths) throws Exception {
        // String str = "";
        Date date = parseDate(dataStr,srcFormat);
    ...
    public static String getDate(String startDate, int day, String format) {
        try {
            Date date = getDate(parse(startDate, format), day);
    ...
    public static Boolean isFirstDayOfMonth(){
        return DateUtils.isFirstDayOfMonth(new Date());
    }
    ...
    public static boolean isLastDayOfMonth() {
        return DateUtils.isLastDayOfMonth(new Date());
    }
    ...
    public static long getSecUnixTimestamp(String dateStr, String dateFormat) {
        return getSecUnixTimestamp(StringToDate(dateStr, dateFormat));
    }

    public static long getSecUnixTimestamp(Date date) {
        long dateTime = date.getTime();
        return (long) (dateTime / 1000);
    }

    public static long getCurDateSecUnixTimestamp() {
        return getSecUnixTimestamp(new Date());
    }
    ...
    public static int getYearDiff(String start, String end, String format) throws ParseException {
        SimpleDateFormat sdf = new SimpleDateFormat(format);
        Calendar c = Calendar.getInstance();
    ...
    /**
     * 返回两个日期相隔的月数，当start比end晚时，返回负数相隔日期
     * @param start 开始日期
     * @param end 结束日期
     ...
     * @throws ParseException
     */
    public static int getMonthDiff(String start, String end, String format) throws ParseException {
        SimpleDateFormat sdf = new SimpleDateFormat(format);
        ...
        return s.after(e) ? -monthsDiff : monthsDiff;
    }
...
//		System.out.println(getAddDate("2016-1-30", "yyyy-MM-dd", 5));
        List<String> list = getBetweenDates("2018-05-01", "2018-05-01", "yyyy-MM-dd");
        ...
        System.out.println(getDate("2017-07-19", -29, "yyyy-MM-dd"));
        System.out.println(getCollectDates("2018-05-01", "2018-05-08", "yyyy-MM-dd"));
        System.out.println(getYesterday("yyyy-MM-dd 00:00:00"));
        System.out.println(getTomorrow("yyyy-MM-dd 00:00:00"));
        System.out.println(DateUtils.parse(DateUtils.getYesterday("yyyyMMdd"), "yyyyMMdd").getTime());
        System.out.println(DateUtils.parse(DateUtils.getTomorrow("yyyyMMdd"), "yyyyMMdd").getTime());
    }
    ...
    /**
     * 获取去年的日期，返回格式yyyyMMdd
     **/
    public static String getLastYear(String date, String format) throws Exception {
        Date d = DateUtils.parseDate(date, format);
        d = DateUtils.addYear(d, -1);
        return getFormatDate(d, format);
    }

    public static String getYesterday(String format) {
        return getFormatDate(addDay(new Date(), -1), format);
    }
    ...
    public static int getMonthLastDay(String yyyyMM) {
        ...
        int year = Integer.parseInt(yyyyMM.substring(0, yyyyMM.length() - 2));
        int month = Integer.parseInt(yyyyMM.substring(yyyyMM.length() - 2, yyyyMM.length()));
    ...
    /**
     * 获取采集日期，如果指定参数为空，默认取昨天
     *
     * @param startDate
     * @param endDate
     * @return
     */
    public static List<String> getCollectDates(String startDate, String endDate, String dateFormat) {
        return getCollectDates(startDate, endDate, dateFormat, 1);
    }
    ...
    public static List<String> getCollectDates(String startDate, String endDate, String dateFormat, int flag) {
        ...
        try {
            ...
            // TODO Auto-generated catch block
            if (flag == 1) {
                startDate = DateUtils.getPreviousDate(dateFormat);
            } else {
                startDate = DateUtils.getCurrentDate(dateFormat);
            }
        }

        try {
            DateUtils.parseDate(endDate, dateFormat);
        } catch (Exception e) {
            // TODO Auto-generated catch block
            if (flag == 1) {
                endDate = DateUtils.getPreviousDate(dateFormat);
            } else {
                endDate = DateUtils.getCurrentDate(dateFormat);
            }
        }

        try {
            if (DateUtils.compare(startDate, endDate, dateFormat)) {
                String dateStr = startDate;
                startDate = endDate;
                endDate = dateStr;
            }

            collectDates.addAll(DateUtils.getBetweenDates(startDate, endDate, dateFormat));
        } catch (Exception e) {
            // 出异常，则默认采昨天的数据
        }

        // 如果未指定采集日期，那么，根据标志取当天或昨天
        if (CollectionUtils.isEmpty(collectDates)) {
            if (flag == 1) {
                collectDates.add(DateUtils.getPreviousDate(dateFormat));
            } else {
                collectDates.add(DateUtils.getCurrentDate(dateFormat));
            }
        }
